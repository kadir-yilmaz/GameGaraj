...
namespace GameGaraj.Catalog.API.Consumers
{
    public class OrderStartedConsumer : IConsumer<OrderStarted>
    {
        ...
        public async Task Consume(ConsumeContext<OrderStarted> context)
        {
            ...
            using (var activity = AppDiagnostics.StartActivity("Decrease Stock"))
            {
                activity?.SetTag("order.id", context.Message.OrderId);
                activity?.SetTag("saga.step", "StockReservation");
                ...
                {
                    activity?.SetStatus(ActivityStatusCode.Error, failReason);
                    ...
                    using (var compActivity = AppDiagnostics.StartActivity("Stock Compensation"))
                    {
                        compActivity?.SetTag("order.id", context.Message.OrderId);
                        compActivity?.SetTag("saga.step", "StockCompensation");
                        ...
                        foreach (var (reservedProduct, reservedQuantity) in reservedProducts)
                        {
                            ...
                            {
                                compActivity?.RecordException(ex);
