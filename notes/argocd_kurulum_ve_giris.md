...
## 8. Karşılaşılabilecek Olası Sorunlar ve Çözümleri

### A. inotify Instance Limit Aşımı Hatası (PhysicalFilesWatcher / FileSystemWatcher)
K3s üzerinde çok sayıda pod (örneğin .NET mikroservisleri, ArgoCD podları, Prometheus, Grafana) aynı anda çalışmaya başladığında, Linux çekirdeğinin `inotify` dosya izleme limiti aşılabilir. Bu durumda podlar `IOException: The configured user limit (128) on the number of inotify instances has been reached` hatasıyla çöker.

#### 1. Durum Tespiti ve Doğrulama Komutları (Host Sunucuda Çalıştırın):
*   **Mevcut inotify limitini sorgulama:**
    ```bash
    cat /proc/sys/fs/inotify/max_user_instances
    # Çıktı varsayılan olarak büyük ihtimalle 128'dir.
    ```
*   **Mevcut dosya izleme (watch) limitini sorgulama:**
    ```bash
    cat /proc/sys/fs/inotify/max_user_watches
    ```
*   **Aktif kaç pod çalıştığını sayma (K3s Kubernetes seviyesinde):**
    ```bash
    kubectl get pods -A | wc -l
    ```
*   **Sunucuda aktif kaç adet inotify izleyicisi kullanıldığını sayma:**
    ```bash
    sudo find /proc/*/fd -lnk 2>/dev/null | grep inotify | wc -l
    ```

#### 2. Çözüm Adımları:
*   **Geçici Çözüm (Hemen uygulamak için):**
    ```bash
    sudo sysctl fs.inotify.max_user_instances=512
    sudo sysctl fs.inotify.max_user_watches=524288
    ```
*   **Kalıcı Çözüm (Reboot sonrası korunması için):**
